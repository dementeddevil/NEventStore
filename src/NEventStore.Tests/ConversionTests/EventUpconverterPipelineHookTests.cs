namespace NEventStore.ConversionTests
{
	...
	public class when_opening_a_commit_that_does_not_have_convertible_events : using_event_converter
	{
		...
		{
			_converted = await EventUpconverter.SelectAsync(_commit, CancellationToken.None);
		}
...
#else
			// in .netcore we return the current assembly, instead of looking at all the assemblies in the folder
			// GetCallingAssembly is not supported
			return new[] { typeof(ConvertingEventConverter).GetTypeInfo().Assembly };
#endif
		}
