...
namespace NEventStore.Client
{
    ...
    public class PollingClient2 : IDisposable
    {
        ...
        private Func<Task<IEnumerable<ICommit>>> _pollingFunc;
        ...
        public PollingClient2(IPersistStreams persistStreams, Func<ICommit, HandlingResult> callback, Int32 waitInterval = 100)
        {
            _persistStreams = persistStreams ??
                              throw new ArgumentNullException(nameof(persistStreams), "PersistStreams cannot be null");
            _commitCallback = callback ?? 
                              throw new ArgumentNullException(nameof(callback), "Cannot use polling client without callback");
            ...
            _pollingThread = Task.Run(InnerPollingLoop);
        ...
        public void ConfigurePollingFunction(string bucketId = null, CancellationToken cancellationToken = default(CancellationToken))
        {
            if (bucketId == null)
                _pollingFunc = () => _persistStreams.GetFromAsync(_checkpointToken, cancellationToken);
            else
                _pollingFunc = () => _persistStreams.GetFromAsync(bucketId, _checkpointToken, cancellationToken);
        }
        ...
        private async Task<bool> InnerPoll()
        {
            ...
            try
            {
                var commits = await _pollingFunc().ConfigureAwait(false);
